...
\paragraph{Pre-Trained Models - Modelos Pre-entrenados.\parencite{pretrained}}
...
\paragraph{Prompt Engineering - Ingeniería de prompts.\parencite{Schulhoff2025}}
...
\paragraph{Common European Framework of Reference for Languages (CEFR) - Marco Común Europeo de Referencia para las Lenguas.\parencite{CouncilEurope2020}}
...
Los Pre-Trained Models han transformado de manera decisiva el campo del NLP. Entrenados sobre grandes cantidades de texto sin etiquetar, estos modelos aprenden representaciones que capturan relaciones semánticas y gramaticales, lo que los convierte en una base versátil para diversas tareas, como por ejemplo: generación de texto y diálogos, resumen y traducción, adaptación de textos, etc.

Existen distintos paradigmas de pre-entrenamiento. Los modelos masked language modeling (por ejemplo, BERT\parencite{Devlin2018}) aprenden a predecir palabras ocultas en una oración (también se los llama encoder-only), mientras que los modelos causales (por ejemplo, GPT\parencite{paper_gpt}) se entrenan para predecir la siguiente palabra en una secuencia (también se los llama decoder-only). En ambos casos, el context window es un factor determinante, ya que limita la cantidad de información que el modelo puede considerar en cada predicción.
...
Una de las formas más accesibles de adaptar un LLM a tareas específicas es a través de la programación con instrucciones en lenguaje natural (prompt). En lugar de modificar sus parámetros internos, se manipula su comportamiento mediante el diseño del prompt.
